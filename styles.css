...
function generateCards() {
    cards = emojis.slice(0, (gridSize * gridSize) / 2);
    cards = [...cards, ...cards];
    shuffle(cards);
    createCardElements();
}
...
function levelUp() {
    ...
    if (level > 1) {
        gridSize += 2; // Increase grid size for higher levels
    }
    resetGame(); // Generate new set of cards on level up
}
...
document.getElementById('pauseGameBtn').addEventListener('click', () => {
    isPaused = !isPaused;
    document.getElementById('pauseGameBtn').textContent = isPaused ? '▶️ Resume' : '⏸️ Pause';
});
